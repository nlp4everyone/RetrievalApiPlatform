...
`app/app.py` also defines `GET /health` (excluded from OpenAPI schema), registers `RequestIDMiddleware`, the global `AppBaseException` handler, and a `lifespan` that brings up tracing → embed model → Postgres (pool + `wait_for_postgres` + table creation) → vector store → MinIO → I/O pool, in that order, then releases the I/O pool, the vector store connections, the Postgres pool, and the dense/sparse embedding providers' HTTP clients below its `yield` — one function, so a client opened above it cannot be forgotten below. It also installs two `uvicorn.access` log filters: `HealthCheckLogFilter` (silences 2xx `/health` probes) and `QuietAccessLogFilter` (silences 2xx list/retrieve/modify calls, which the service layer already logs; create/delete/search stay visible, and every non-2xx line still shows up).
...
`IngestionContext` is one mutable dataclass threaded through every stage — inputs (`vector_store_id`, `file_id`, `file_metadata`, chunking params), progressively filled results (`raw_bytes`/`content_sha256` → `text` → `chunks` → `num_inserted`, plus `parsed_from_cache` and the `splitter`/`splitter_detected` pair `ChunkStage` resolves), and a free-form `metrics` dict for numbers that belong on a span but not in pipeline state. Convenience properties (`filename`, `file_extension`, `storage_bucket`, `storage_path`) read out of `file_metadata`.

| Stage | Does | Span notes |
|---|---|---|
| `DownloadStage` | Fetches bytes from MinIO under `get_storage_semaphore()`, transfer on `get_io_executor()`, then hashes them on `get_cpu_executor()` | bucket/path, filename, `file.size_bytes`, `file.content_sha256` |
| `ParseStage` | Looks the parse cache up first; on a miss, `ParsingService.parse(bytes, ext)` → text (Markdown) | provider **slug** that handled it, `parser.cache` (`hit`/`miss`), `text.num_chars` |
| `PersistTextStage` | Writes that Markdown back to `PARSED_TEXT_BUCKET`, best effort; skipped on a cache hit | bucket + `storage.path`, `parsed_text.saved`, `parsed_text.size_bytes` |
| `ChunkStage` | Resolves the splitter (request's, else detected from the text), then `ChunkingService.split_text(text)` → chunks, on `get_cpu_executor()` | `chunk.strategy`, `chunk.detected`, `chunk.size`, `chunk.overlap`, `chunks.count`, `chunks.avg_chars` |
...
`build_ingestion_pipeline(...)` is the single place deciding which stages run and in what order. It fixes the splitter only when the request named one; the `ChunkingService` itself is built inside `ChunkStage` rather than at startup or per pipeline, because sizing comes from the create request and the splitter may only be knowable once the document has been parsed.
...
Resolving the type this way already checks `collection_exists()` once (through `supports_sparse()`), and the retriever built for that type needs the same answer. Rather than let it ask again, `search` checks it once up front (only when the branch above needs it — a pinned dense search skips this and lets the retriever check fresh) and threads the result through `resolve_search_type`/`hybrid_unavailable_reason`/`supports_sparse` and into `DenseRetriever`/`HybridRetriever` as an optional parameter, saving a round-trip to the backend on the common path without touching `ensure_collection()`'s own (differently-purposed) re-check during ingestion.
...
| `chunking/` | `BaseChunkingProvider` | `ChonkieProvider`, `LangchainProvider` | the request's `chunking_splitter`, or the splitter detected from the document — resolved through `registry.py` |
...
`ChunkingService.for_strategy(strategy, chunk_size, chunk_overlap)` builds the splitter for one strategy through `registry.py`, and exposes `chunk_size`, `chunk_overlap` and `async split_text(text)`. Five strategies are registered, each mapping to exactly one implementation: `recursive`, `token` and `sentence` on Chonkie, `character` and `markdown` on `langchain_text_splitters`. The registry is a total function over the enum, checked at import — adding a value without its entry fails at startup rather than with a `KeyError` in the worker.

Which one runs is either named by the request (`chunking_splitter`) or read off the document by `detection.py`: `markdown` when the upload was `.md`/`.markdown` or the parsed text carries at least two `h1`–`h4` headings, `recursive` otherwise. The markdown splitter cuts on headings first and prefixes every chunk with its heading path (`Guide > Billing > Refunds`), charging that prefix against the chunk budget, then hands any section still over budget to a recursive pass.

Sizing stays orthogonal: `chunking_strategy` (`"auto"`/`"static"`) only decides `chunk_size`/`chunk_overlap`, and under `"auto"` neither is pinned, so each strategy's own defaults apply — 800/400 for the window splitters, 1200/120 for markdown, whose chunks are whole heading sections rather than fixed windows. `recursive` has no overlap knob at all (`ChunkingStrategy.supports_overlap`), so asking for one alongside an explicit `chunking_splitter="recursive"` is a 422 rather than a silent drop; an overlap that would meet or exceed `chunk_size` is bounded to half of it instead of raising inside the worker. `tokenizer="character"` throughout means `max_chunk_size_tokens` is still counted in characters, not tokens (see [Design Decisions](DESIGN_DECISIONS.md#known-gaps)). What actually ran is written back onto the context, reported on the `chunk` span, and merged into the store's `chunking_strategy.resolved`.
...
1. **pydantic-settings** (`settings.py`) — reads `.env` / real environment variables. Required fields (no default) are the API keys and the Postgres/MinIO/Langfuse credentials; the vector store's are **conditionally** required, checked by `validate_vector_store_credentials` for `VECTOR_STORE_PROVIDER` only (`QDRANT_URL` + `QDRANT_API_KEY`, or `MILVUS_URI`) — a Qdrant-only deployment never has to fill in Milvus, and filling a backend in is itself what connects it (`enabled_vector_store_providers`). Validators enforce `API_VERSION` starts with `v`, ports are positive, secrets are non-empty, `LOG_LEVEL`/`LOG_FORMAT` are in an allowed set, and each of `EMBEDDING_PROVIDER` / `SPARSE_EMBEDDING_PROVIDER` / `PDF_PARSER_PROVIDER` / `VECTOR_STORE_PROVIDER` names a known backend — so a typo fails at startup, not at first use. `_VECTOR_STORE_REQUIRED_SETTINGS` is one dict serving as both the allowed-value set and the per-backend credential list, so adding a backend is a single edit.
