...
async def load_and_chunk_file(minio_client: Minio,
                              file_metadata: dict[str, Any],
                              chunk_size: Optional[int]) -> list[str]:
    ...
    file_bytes: Optional[bytes] = await MinioFileStore.download_file(minio_client=minio_client,
                                                                     bucket_name=file_metadata.get("minio_bucket"),
                                                                     file_path=file_metadata.get("minio_path"))
    if file_bytes is None:
        raise ValueError(f"Failed to download file: {file_metadata.get('minio_path')}")
